...
4. **No Claude Code hook currently addresses any of this.**
   `github-sdlc-planning`'s three hooks (`session-start.mjs`, fetches open
   milestones and injects them as session context; `validate-mif.mjs`, MIF
   frontmatter conformance; `confirm-mutation.mjs`, a confirmation prompt
   before a board mutation) do not watch for or nudge a status transition
   of any kind.
