 ...
 * fallback. Branch-protection/rulesets/custom-properties writes require
 * `repo`/`admin:org` scope depending on target; a missing-permission
 * response from the real endpoint surfaces as a plain github_api_error. */
export function resolveToken(execImpl: ExecFileSyncFn = defaultExecFileSync): string {
  ...
  throw new RepoConfigError('missing_scope', 'No GitHub token available. Set GITHUB_TOKEN, or run `gh auth login`.');
}
...
async function handleResponse(res: Response): Promise<unknown> {
  ...
  if (!res.ok) {
    ...
    throw new RepoConfigError('github_api_error', `GitHub API error ${res.status}: ${text}`, { status: res.status });
  }
