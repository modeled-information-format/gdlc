 ...
 * fallback. No mutation-pacing governor here (unlike the sibling plugins'
 * github-client.ts) -- every tool in this plugin issues GET requests only,
 * since the underlying REST endpoints (traffic, stats, community profile,
 * dependency-graph) have no write counterpart at all. */
export function resolveToken(execImpl: ExecFileSyncFn = defaultExecFileSync): string {
  ...
  throw new InsightsError('missing_scope', 'No GitHub token available. Set GITHUB_TOKEN, or run `gh auth login`.');
}
...
async function handleResponse(res: Response): Promise<unknown> {
  ...
  if (!res.ok) {
    ...
    throw new InsightsError('github_api_error', `GitHub API error ${res.status}: ${text}`, { status: res.status });
  }
