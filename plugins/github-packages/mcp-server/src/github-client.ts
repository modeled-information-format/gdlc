 ...
 * fallback. Package delete/restore requires the token's identity to hold
 * org-level `read:packages`/`delete:packages` scope depending on the
 * operation; a missing-permission response surfaces as a plain
 * github_api_error. */
export function resolveToken(execImpl: ExecFileSyncFn = defaultExecFileSync): string {
  ...
  throw new PackagesError('missing_scope', 'No GitHub token available. Set GITHUB_TOKEN, or run `gh auth login --scopes read:packages,delete:packages`.');
}
...
async function handleResponse(res: Response): Promise<unknown> {
  ...
  if (!res.ok) {
    ...
    throw new PackagesError('github_api_error', `GitHub API error ${res.status}: ${text}`, { status: res.status });
  }
