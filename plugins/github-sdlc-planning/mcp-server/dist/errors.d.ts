export type PlanningErrorCode = 'limit_exceeded' | 'missing_scope' | 'resolve_issue_id' | 'resolve_project_id' | 'unknown_issue_type' | 'rate_limited' | 'github_api_error' | 'confirmation_required' | 'missing_board_config' | 'missing_destination' | 'repo_not_allowed' | 'invalid_config';
...
export declare class PlanningError extends Error {
    ...
    toJSON(): {
        error: PlanningErrorCode;
        message: string;
    } & PlanningErrorDetails;
}
export declare function isPlanningError(err: unknown): err is PlanningError;
