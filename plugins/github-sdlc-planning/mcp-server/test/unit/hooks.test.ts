...
// gdlc#227 / ADR-0008: a fresh tmpdir with no `.config/gdlc/config.yml` of
// its own, and XDG_CONFIG_HOME pointed at a nonexistent global path within
// it, so confirm-mutation.mjs's upward search can't accidentally climb into
// (and pick up a real `packs:` override from) whatever ancestor directories
// the machine actually running these tests happens to have -- e.g. a real
// workspace-root `.config/gdlc/config.yml` several levels above this repo's
// own checkout, or (post-#227 fix) this repo's own `.config/gdlc/config.yml`
// once it legitimately defines `packs:`. Every test below that isn't itself
// testing the skipMutationConfirm pack needs this so it observes
// confirm-mutation.mjs's fail-closed default (still asks) regardless of the
// host machine's real directory tree.
