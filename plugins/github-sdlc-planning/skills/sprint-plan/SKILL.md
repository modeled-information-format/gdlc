---
description: Plan a sprint from the open backlog, team size, and cadence — select candidate issues, assign the Sprint iteration field, and set Story Points. Use when the user asks to "plan the next sprint", "fill sprint N", or "what should we work on this sprint". Skill only — there is no matching Agent; invoke via the Skill tool, never the Agent tool's subagent_type (which will report "not found").
